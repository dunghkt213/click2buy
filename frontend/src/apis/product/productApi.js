...
async function getAll(query) {
    ...
    if (query?.brands) {
        const brandsValue = Array.isArray(query.brands) ? query.brands.join(',') : query.brands;
        if (brandsValue)
...
async function getAllBySeller(query) {
    ...
    const response = await request(`/products/seller${queryString ? `?${queryString}` : ''}`, {
        method: 'GET',
        requireAuth: true,
    });
...
async function updateStock(id, amount) {
    const data = await request(`/products/${id}/stock`, {
        method: 'PATCH',
