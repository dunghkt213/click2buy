...
// Layout Components
import { Header } from './components/layout/Header'; // Thêm dấu chấm và gạch chéo ./
...
// Flying Icon
import { FlyingIcon, FlyingIconConfig } from './components/FlyingIcon';
...
// Review Modal types
import { ReviewData } from './components/review/ReviewModal';
...
export default function App() {
  ...
  // State cho profile page
  const [isProfilePageOpen, setIsProfilePageOpen] = useState(false);

  // State cho store registration modal
  const [isStoreRegistrationOpen, setIsStoreRegistrationOpen] = useState(false);
  
  // State cho order success modal
  const [isOrderSuccessOpen, setIsOrderSuccessOpen] = useState(false);
  const [lastOrderNumber, setLastOrderNumber] = useState<string>('');
  const [ordersDefaultTab, setOrdersDefaultTab] = useState<'all' | 'pending' | 'confirmed' | 'shipping' | 'completed' | 'cancelled' | 'refund'>('all');
  ...
  const handleOrdersClick = () => {
    setOrdersDefaultTab('all'); // Reset về tab "Tất cả"
  ...
  const handleCheckout = (checkoutData: any) => {
    ...
    // Mở order success modal
    setIsOrderSuccessOpen(true);
  };

  // Order success handlers
  const handleViewOrderFromSuccess = () => {
    setIsOrderSuccessOpen(false);
    setOrdersDefaultTab('pending'); // Mở tab "Chờ xác nhận"
    setIsOrdersPageOpen(true);
  };

  const handleContinueShopping = () => {
    setIsOrderSuccessOpen(false);
    handleLogoClick(); // Quay về trang chủ
  };
  ...
  const handleReview = (orderId: string, reviewData: ReviewData) => {
    console.log('Review submitted for order:', orderId, reviewData);
    alert(`Cảm ơn bạn đã đánh giá!\n\nSản phẩm: ${reviewData.productRating} sao\nVận chuyển: ${reviewData.shippingRating} sao\nNgười bán: ${reviewData.sellerRating} sao\n\nNhận xét: ${reviewData.comment}\nẢn danh: ${reviewData.isAnonymous ? 'Có' : 'Không'}`);
  };
  ...
  // Cart click - kiểm tra đăng nhập trước
  ...
  const handleStoreRegistration = (storeData: Omit<StoreInfo, 'id' | 'rating' | 'totalReviews' | 'totalProducts' | 'followers' | 'joinedDate'>) => {
    ...
    // Load sample data
    setStoreProducts(initialStoreProducts);
  ...
  const handleUpdateBankAccount = (data: { accountNumber: string; bankName: string }) => {
    console.log('Update bank account:', data);
  };
      ...
      {isMyStorePageOpen ? (
        <>
          <Header 
            ...
            hasStore={hasStore}
            onMyStoreClick={handleMyStoreClick}
          />
        ...
        <OrdersPage
            ...
            onContactShop={handleContactShop} onReview={function (orderId: string): void {
