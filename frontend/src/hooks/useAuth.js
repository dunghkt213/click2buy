...
import { authStorage, authService } from '../apis/auth';
import { userService, normalizeUser } from '../apis/user';
export function useAuth() {
    const [user, setUser] = useState(() => authStorage.getUser());
    const [isLoggedIn, setIsLoggedIn] = useState(() => !!(authStorage.getUser() && authStorage.getToken()));
    ...
    const fetchAndUpdateUserInfo = useCallback(async (userId) => {
        try {
    ...
    const handleLoginSuccess = useCallback(async ({ user: userData, accessToken }) => {
    ...
    const handleRegisterSuccess = useCallback(async ({ user: userData, accessToken }) => {
        setIsLoggedIn(true);
        setUser(userData);
        authStorage.save(userData, accessToken);
        // Fetch đầy đủ thông tin user từ API (bao gồm role)
    ...
    const handleAuthCallbackSuccess = useCallback(async (userData, token) => {
        setIsLoggedIn(true);
        setUser(userData);
