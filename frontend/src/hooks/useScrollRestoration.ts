 ...
 */
function getScrollPosition(pathname: string): number | null {
  try {
    const stored = sessionStorage.getItem(SCROLL_POSITIONS_KEY);
 ...
 */
function clearScrollPosition(pathname: string) {
  try {
    const stored = sessionStorage.getItem(SCROLL_POSITIONS_KEY);
    if (!stored) return;
    const positions: ScrollPositions = JSON.parse(stored);
 ...
 */
export function useScrollRestoration(enabled: boolean = true, delay: number = 100) {
