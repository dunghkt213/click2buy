...
export function useChat({ userId, autoConnect = true }: UseChatOptions) {
  ...
  useEffect(() => {
    ...
    const unsubscribeConversations = chatService.onConversations((convs: Conversation[]) => {
    ...
    const unsubscribeTyping = chatService.onTyping((data: TypingStatus) => {
      setIsTyping((prev) => ({
        ...prev,
    ...
    const unsubscribeError = chatService.onError((error: ChatError) => {
    ...
    const unsubscribeUnreadCount = chatService.onUnreadCount((count: number) => {
      setUnreadCount(count);
    });

    // Handle messages history
    const unsubscribeMessagesHistory = chatService.onMessagesHistory((messages: ChatMessage[]) => {
      ...
      setMessages((prev) => {
        ...
        const normalizedMessages = messages.map(msg => ({
          ...msg,
          id: msg.id || (msg as any)._id || '', // Normalize id field
  ...
  const sendTyping = useCallback(
    (receiverId: string, typing: boolean) => {
      if (!isConnected) return;
  ...
  const loadMessages = useCallback(
    (conversationId: string, limit: number = 50, skip: number = 0) => {
      if (!isConnected) return;
