...
const kafka_helper_1 = require("../utills/kafka.helper");
let ReviewGateway = class ReviewGateway {
    ...
    create(dto, auth) {
        ...
        console.log('📤 Sending Kafka message: ', { dto, auth });
        return (0, kafka_helper_1.handleKafkaRequest)(this.kafka, 'review.create', { dto, auth });
    }
    findAll(q) {
        return (0, kafka_helper_1.handleKafkaRequest)(this.kafka, 'review.findAll', { q });
    }
    findOne(id) {
        return (0, kafka_helper_1.handleKafkaRequest)(this.kafka, 'review.findOne', { id });
    }
    update(id, dto, auth) {
        console.log('📤 Sending Kafka update message:', { id, dto, auth });
        return (0, kafka_helper_1.handleKafkaRequest)(this.kafka, 'review.update', { id, dto, auth });
    }
    remove(id, auth) {
        return (0, kafka_helper_1.handleKafkaRequest)(this.kafka, 'review.delete', { id, auth });
    }
};
...
__decorate([
    ...
    (0, swagger_1.ApiBody)({
        schema: {
            type: 'object',
            properties: {
                productId: { type: 'string', example: '6734abc123def45678901234' },
